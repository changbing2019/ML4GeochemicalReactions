...
def main():
    ...
    wholeDataset = pd.concat([trainData[0],trainData[1],trainData[2]],axis=0)
    inputColNames =['b(CaO)','b(SiO2)','b(H2O)']
    inputColAqSiNames =['b(CaO)','b(SiO2)','b(H2O)','pH']    

    # read the test data
    InsFile = os.path.join(dataFolder, 'dataset5K_test.csv')
    data =pd.read_csv(InsFile)
    trainAndTest = pd.concat([wholeDataset,data],axis=0)

    X = trainAndTest.loc[:,inputColNames]
    scaler = StandardScaler().fit(X)
    print(scaler.__dict__)
    pickle.dump(scaler, open('scaler.pkl', 'wb'))

    X = trainAndTest.loc[:,inputColAqSiNames]
    scalerSi = StandardScaler().fit(X)
    pickle.dump(scalerSi, open('scalerSi.pkl', 'wb'))
    print(scalerSi.__dict__)
    ...
    for i, data in enumerate(trainData):
        ...
        for jcol,col in enumerate(['pH','nSi(aq)']):  # data.columns[4:]): 
            ...
            if percnt <1.0:
               ...
               tempdf = pd.DataFrame({'var':[col],'const':[(Y.max()+Y.min())*0.5]})
            ...
            if col == 'nSi(aq)' and group =='group2':  # need to add pH as another input to the model 
                dataX = data.loc[:,inputColAqSiNames ]
                X = dataX.values
                Y = dataY.values
                X_scaled = scalerSi.transform(X)  
    ...
    ModelTrainSummary.to_csv('ModelTrainSummary.csv')
